...
if (Test-Path $sdPath) {
    ...
    if ($sdSize -gt 1GB) {
        $sizeGB = [math]::Round($sdSize/1GB, 2)
...
# Check 7: Windows Update Log for specific errors
Write-Host "Checking Windows Update logs for 0x80240021..." -ForegroundColor Yellow
try {
    $logPath = "C:\Windows\Logs\DISM\dism.log"
    if (Test-Path $logPath) {
        $recentLogs = Get-Content $logPath -Tail 100 | Select-String "0x80240021"
        if ($recentLogs) {
            $issues += "❌ Found 0x80240021 errors in DISM log"
            $fixes += "FIX_LOGS"
        }
    }
} catch {
...
try {
    $testUpdate = Get-WUList -ErrorAction SilentlyContinue 2>$null
    if ($?) {
        Write-Host "✅ Windows Update is functional" -ForegroundColor Green
        $updateWorking = $true
    } else {
        # Alternative test - try to enable a simple feature
        $testResult = & dism /online /get-features | Select-String "NetFx3"
        if ($testResult) {
    ...
    Write-Host "⚠️  Cannot fully verify Windows Update (but fixes were applied)" -ForegroundColor Yellow
